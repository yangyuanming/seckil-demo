...
var seckill = {
    ...
    URL : {
        ...
        execution : function(seckillId) {
            return '/seckill/' + seckillId + '/execution' ;
        }
    },
    // 处理秒杀逻辑
        ...
        $.post(seckill.URL.exposer(seckillId), {}, function(result) {
            ...
            if (result && result.code==200) {
                ...
                if (exposer['exposed']) {
                    ...
                    var killUrl = seckill.URL.execution(seckillId);
                    ...
                    $('#killBtn').one('click', function() {
                        ...
                        // 2.发送秒杀请求
                        $.post(killUrl,{md5:md5}, function(result) {
                            if (result&& result.code==200) {
                                var msg=result.msg;
                                // 3.显示秒杀成功信息
                                node.html('<span class="label label-success">' + "用户"+result.data.successKilled.userPhone+msg + '</span>');
                            }
                            if (result && result.code!=200) {
                                var msg=result.msg;
                                // 4.显示秒杀失败原因
